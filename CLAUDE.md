...
└── js/
    ...
    ├── tab-report-category.js   # Spend by Category report tab
    ├── tab-report-payee.js      # Spend by Payee report tab
    ├── tab-report-cashflow.js   # Monthly Cash Flow report tab
...
The **Monthly cash flow** command first asks the user to select a time frame: Year to date, Previous year, or Specific year (prompts for a 4-digit year). It then opens a closeable Report tab (`rpt-cf-N`) with a horizontally scrollable plain-HTML table. Columns run from January through the last month that has transaction data (capped at the current month for the current year, December for past years). Rows are grouped into three sections — **Income**, **Expense**, **Other** — shown in that order; each group has a coloured header band. Within each group one row per active category shows the `SUM(Amount)` per month (income positive, expense negative, per the DB storage convention). A **Total** column on the right sums each category across all shown months. A **Net Cash Flow** footer row sums all categories per month (algebraic total = net change in cash). Zero-value cells display `—`; negative values are shown in accounting notation `(x.xx)` in red. Tab IDs are `rpt-cf-N`.
...
The two seed categories — **Unassigned** (Category_ID=1) and **Transfer** (Category_ID=2) — are protected: attempting to delete or deactivate either (via the Delete button or the Delete key) shows an error message and takes no action. These categories are required by the application's import and transfer logic.
...
- Application version is defined as the global constant `APP_VERSION = '1.0'` at the top of `app.js`; displayed as `v{version}` prefix in the status bar (e.g. `v1.0 money_202606142107.db`)
- Change counter: `DB.run()` increments `_changeCount` for every INSERT/UPDATE/DELETE; `DB.setChangeListener(fn)` registers a callback fired after each write; `DB.getChangeCount()` / `DB.resetChangeCount()` manage the count; `app.js` registers a listener that calls `_renderStatus()` to append `(+N)` to the status bar; count resets to 0 on `createNew()`, `loadFromBytes()`, and after a successful save
...
- `js/tab-categories.js` — Categories tab: Tabulator table (Name, Type, Last Transaction) + 3-column form (left: Name; middle: Type dropdown Expense/Income/Other; right: New/Delete/Accept buttons); New resets Type to Expense; Accept saves Name and Type; category name hyperlinks to filtered Transactions tab; `loadFromBytes` migration adds Type column with DEFAULT 'Expense' for existing databases; `handleDelete` blocks deletion of Category_ID 1 (Unassigned) and 2 (Transfer) with an error message
...
- `js/tab-report-cashflow.js` — Monthly Cash Flow report (`CashFlowReport`): `open()` shows a custom radio-button timeframe dialog (Year to date / Previous year / Specific year; Specific year chains into `Dialogs.prompt()` for 4-digit year); queries `SUM(Amount)` per category per month joining Transactions → Categories (all types, Valid=1); month columns span Jan to last month with data (capped at current month for current year, Dec for past years); rows grouped Income → Expense → Other with coloured group header bands; Total column sums each category across months; Net Cash Flow footer row sums all categories per month; zero cells show `—`, negatives shown as `(x.xx)` in red; tab IDs `rpt-cf-N`; custom dialog uses `modal-container` / `modal-backdrop` pattern matching `Dialogs._show()/_hide()`
- `app.css` — full application styles; `#tab-strip` (flex row wrapper for tab bar + refresh button); `#tab-bar` (scrollable inner flex container, `flex: 1`); `#tab-refresh` (SVG icon button anchored at right end of strip); 3-column form layout (`.form-col`, `.form-col-btns`, `.form-field`, `.btn-spacer`); `.form-readonly` for informational fields; `.amt-cur` for Amount+Currency inline pair; `.qry-*` styles for Query tab layout; `.dlg-xl` (680px wide dialog), `.csv-map-grid` (4-column: 120px field label | 1fr CSS dropdown | 1fr pattern | 48px Neg) / `.csv-map-hdr` / `.csv-map-field` / `.csv-map-src` / `.csv-map-pattern` / `.csv-neg-label` / `.csv-map-err` for CSV column mapping dialog; `.dlg-checkbox-label` for checkbox rows in dialogs; `.rpt-*` styles for Report tab layout (scrollable padded area, dark header row, alternating body rows, bold grand-total footer row); `.rpt-cf-*` styles for Monthly Cash Flow table (`.rpt-cf-scroll` horizontal-scroll wrapper, `.rpt-cf-table` no-wrap table, `.rpt-cf-hdr` dark header, `.rpt-cf-cat` left-aligned category col, `.rpt-cf-num` right-aligned number cols, `.rpt-cf-rowtotal` bold total col with left border, `.rpt-cf-grp-cell` coloured group header band, `.rpt-cf-netrow` bold footer row, `.rpt-cf-neg` red accounting-notation negatives)
